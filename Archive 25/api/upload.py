...
def _dsid(client, fn):
    return generate_dataset_id(client, "LOCAL", fn)
...
async def upload_list(
    ...
    """
    query = db.query(MasterConfigAuthoritative).filter(MasterConfigAuthoritative.source_type == "LOCAL")
    
    if client_name:
        query = query.filter(MasterConfigAuthoritative.client_name == client_name)
    ...
    for record in records:
        files.append({
            ...
            "target_layer_silver": record.target_layer_silver,
            "is_active": record.is_active,
            "last_seen_batch": record.last_seen_batch,
